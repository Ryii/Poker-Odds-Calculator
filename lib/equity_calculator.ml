(* Real-time equity and pot odds calculator *)
...
type equity_result = {
  ...
  hand_strength: float;  (* 0-1, how strong is current hand *)
  potential: float;      (* 0-1, how likely to improve *)
}
...
type pot_odds = {
  pot_size: float;
  bet_to_call: float;
  pot_odds_ratio: float;     (* bet_to_call / (pot_size + bet_to_call) *)
  required_equity: float;    (* minimum equity needed to call *)
  implied_odds_factor: float; (* multiplier for future bets *)
}
...
let calculate_equity ~hero_hand ~villain_range ~board ~iterations =
  ...
  total := iterations;
  {
    ...
    potential = 0.0;  (* Will be calculated separately *)
  }

(* Fast exact equity for heads-up all-in situations *)
...
(* Calculate implied odds based on opponent tendencies *)
let estimate_implied_odds ~position ~opponent_vpip ~opponent_aggression ~stack_sizes =
  (* Simple model: more aggressive opponents give better implied odds *)
...
(* Hand potential calculation - how likely to improve *)
let calculate_hand_potential ~hero_hand ~board =
  if List.length board >= 5 then 0.0  (* No cards to come *)
  else
    let hero_cards = match hero_hand with (c1, c2) -> [c1; c2] in
    let current_hand = evaluate_hand (hero_cards @ board) in
    
    (* Count outs based on current hand *)
    let outs = match current_hand with
      | HighCard _ -> 6  (* Pair outs *)
      | Pair _ -> 5      (* Two pair or trips *)
      | TwoPair _ -> 4   (* Full house *)
      | _ -> 2           (* General improvement *)
    in
    
    let cards_to_come = 5 - List.length board in
    let cards_in_deck = 52 - 2 - List.length board in
    
    (* Approximate probability of hitting an out *)
    1.0 -. (float (cards_in_deck - outs) /. float cards_in_deck) ** float cards_to_come

(* Convert hand range string to card list *)
let parse_range str =
  (* Simple range parser - can be expanded *)
  let all_cards = Card.full_deck () in
  match str with
  | "random" | "100%" -> all_cards
  | "top10%" -> 
      (* Approximate top 10% of hands *)
      List.filter (fun c -> 
        let r = Card.rank_to_int c.Card.rank in
        r >= 9  (* Ten or better *)
      ) all_cards
  | _ -> all_cards  (* Default to any two cards */

(* Helper to check if we should call based on pot odds *)
let should_call equity pot_odds =
  equity.hand_strength >= pot_odds.required_equity

(* Get a readable description of pot odds *)
